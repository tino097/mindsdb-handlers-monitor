...
def mindsdb_connection():
    ...
    """
    from mindsdb.integrations.handlers.mssql_handler.mssql_handler import (
        SqlServerHandler,
    )
    ...
    logger.info(
        f"🔗 Connecting to MS SQL Server at {connection_params['host']}:{connection_params['port']}"
    )
    ...
    ), f"❌ Failed to connect to MS SQL Server: {connection_status.error_message}"
...
def test_connection(mindsdb_connection):
    ...
    ), f"Connection check failed: {response.error_message}"
...
def test_tpch_q10_returned_item_reporting(mindsdb_connection):
    ...
    logger.info(
        f"✅ TPC-H Q10 completed: {len(response.data_frame)} customers with returns"
    )
