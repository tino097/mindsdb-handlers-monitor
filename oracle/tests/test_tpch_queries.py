...
This test suite exercises the twenty‑two TPC‑H benchmark queries
against an Oracle database connected through MindsDB.  It follows
the structure and conventions used in the more comprehensive
PostgreSQL test suite: common fixtures manage database setup and
teardown, a helper function dispatches SQL via the MindsDB HTTP
API, and tests are grouped within a class and tagged with a
pytest marker for easy filtering.

The tests assume that the underlying Oracle tables have been
loaded with the full TPC‑H dataset (via SQL*Loader or
external tables) and that the user specified in the environment
owns those tables.  Each query references the database by name
(`oracle_tpch`) without a schema prefix, which matches
MindsDB’s requirement that table names have a single part.
"""
...
# Configure a module-level logger.  These tests emit informative messages
# during setup and teardown and when individual tests start and finish.  To
...
# Base URL for the MindsDB HTTP API.  Tests can override this value by
# defining the MINDSDB_API_URL environment variable.  The default assumes
# MindsDB is running locally on port 47334 (the typical development port).
MINDSDB_API_URL = os.getenv("MINDSDB_API_URL", "http://localhost:47334")

# Name of the MindsDB database that will be created for these tests.  If you
# prefer a different name, set the ORACLE_TPCH_DB environment variable prior
# to running pytest.  The CREATE DATABASE statement uses IF NOT EXISTS to
# avoid errors on repeated runs.
...
def mindsdb_connection(verify_mindsdb_ready: str) -> str:
    ...
    sql = f"""
        CREATE DATABASE IF NOT EXISTS {ORACLE_TPCH_DB}
        WITH ENGINE = 'oracle',
        PARAMETERS = {{
            {param_str}
        }};
    ...
    try:
        ...
        try:
            cleanup_sql = f"DROP DATABASE IF EXISTS {ORACLE_TPCH_DB};"
            execute_sql_via_mindsdb(cleanup_sql, timeout=30)
            logger.info("🧹 MindsDB Oracle database cleaned up")
        except Exception:
